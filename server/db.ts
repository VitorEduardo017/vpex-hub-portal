...
export async function getFinancialSnapshotsForPeriod(
  companyId: number,
  startDate: string,
  endDate: string
) {
...
export async function getDashboardKPIs(companyId: number) {
  ...
  // Get previous month boundaries for comparison
  ...
  const currentSnapshots = await getFinancialSnapshotsForPeriod(
    companyId,
    firstOfMonth,
    today
  );
  ...
  const prevSnapshots = await getFinancialSnapshotsForPeriod(
    companyId,
    prevFirstOfMonth,
    prevEndDate
  );
  ...
  const goalResult = await db
    ...
    .where(
      and(
        ...
        eq(monthlyGoals.monthDate, firstOfMonth)
      )
    )
    .limit(1);
  ...
  // Today's snapshot
  const todaySnapshot = currentSnapshots.find(
    (s) => s.snapshotDate === today
  );
  ...
  const yesterdaySnapshot = currentSnapshots.find(
    (s) => s.snapshotDate === yesterdayStr
  );

  // Aggregate current month revenue
  const currentMonthRevenue = currentSnapshots.reduce(
    (sum, s) => sum + s.revenueCents,
    0
  );
  const prevMonthRevenue = prevSnapshots.reduce(
    (sum, s) => sum + s.revenueCents,
    0
  );
  ...
  const avgTicketCurrent =
    currentSnapshots.length > 0
      ? currentSnapshots.reduce((sum, s) => sum + s.avgTicketCents, 0) /
        currentSnapshots.length
      : 0;
  const avgTicketPrev =
    prevSnapshots.length > 0
      ? prevSnapshots.reduce((sum, s) => sum + s.avgTicketCents, 0) /
  ...
  return {
    ...
    revenuePercent:
      goal?.revenueGoalCents && goal.revenueGoalCents > 0
        ? Math.round((currentMonthRevenue / goal.revenueGoalCents) * 100)
        : 0,
    daysRemaining,
    remainingCents: Math.max(
      0,
      (goal?.revenueGoalCents ?? 0) - currentMonthRevenue
    ),
    dailyAvgNeededCents:
      daysRemaining > 0
        ? Math.round(
    ...
    todayRevenueChange: yesterdaySnapshot
      ? calculateChange(
          todaySnapshot?.revenueCents ?? 0,
          yesterdaySnapshot.revenueCents
        )
      : 0,
    avgTicketCents: Math.round(avgTicketCurrent),
    avgTicketChange: calculateChange(avgTicketCurrent, avgTicketPrev),
    cmvPercent: todaySnapshot?.cmvPercent
      ? parseFloat(String(todaySnapshot.cmvPercent))
      : 0,
    idealCmvPercent: goal?.idealCmvPercent
      ? parseFloat(String(goal.idealCmvPercent))
      : 38,
    netMarginPercent: todaySnapshot?.netMarginPercent
      ? parseFloat(String(todaySnapshot.netMarginPercent))
      : 0,
    netMarginChange: calculateChange(
      todaySnapshot?.netMarginPercent
        ? parseFloat(String(todaySnapshot.netMarginPercent))
        : 0,
      prevSnapshots.length > 0
        ? parseFloat(
            String(prevSnapshots[prevSnapshots.length - 1].netMarginPercent)
          )
        : 0
    ),
...
export async function bulkInsertFinancialSnapshots(
  data: InsertFinancialSnapshot[]
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getRevenueByChannelForPeriod(
  companyId: number,
  periodDate: string
) {
  const db = await getDb();
...
export async function bulkInsertRevenueByChannel(
  data: InsertRevenueByChannel[]
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  if (data.length === 0) return;
...
export async function getMarketingMetricsForPeriod(
  companyId: number,
  periodDate: string
) {
  const db = await getDb();
...
export async function bulkInsertMarketingMetrics(
  data: InsertMarketingMetric[]
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  if (data.length === 0) return;
...
export async function getLogisticsMetricsForPeriod(
  companyId: number,
  startDate: string,
  endDate: string
) {
...
export async function getHrMetricsForPeriod(
  companyId: number,
  startDate: string,
  endDate: string
) {
...
export async function getActiveAlerts(companyId: number) {
  ...
  return db
    ...
    .from(alerts)
    .where(
      and(eq(alerts.companyId, companyId), eq(alerts.isActive, true))
    )
...
// ═══════════════════════════════════════════════════════════════
// ADMIN — Aggregated queries for VPEX admin panel
// ═══════════════════════════════════════════════════════════════
...
export async function getAdminOverview() {
  ...
  const activeCount = allCompanies.filter(
    (c) => c.status === "ativo"
  ).length;
  const avgHealth =
    allCompanies.length > 0
      ? Math.round(
          allCompanies.reduce((sum, c) => sum + c.healthScore, 0) /
            allCompanies.length
        )
      : 0;
  ...
  const byStatus = {
    ...
    inadimplente: allCompanies.filter((c) => c.status === "inadimplente")
      .length,
